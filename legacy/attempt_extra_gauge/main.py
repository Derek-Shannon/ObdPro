...
class MainScreen(tk.Frame):
    ...
    def setup_gauges(self):
        ...
        # This will be used in case we want to show a message to the user that something went wrong
        self.gauges.clear()
        ...
        for i in range(4):
            ...
            # Determine which gauge class to use based on the checkbox state
            if self.app.legacy_gauge_selection[i]:
...
class SettingsScreen(tk.Frame):
    ...
    def setup_ui(self):
        ...
        # Remove weight from the first two columns.
        self.columnconfigure(0, weight=0)
        self.columnconfigure(1, weight=0)
        self.columnconfigure(2, weight=0)
        # Add a new column with a weight of 1 to push everything to the left.
        ...
        for i in range(4):
            ...
            # The label sticks to the right side of its cell to get closer to the combobox.
            label.grid(row=i, column=0, padx=10, pady=5, sticky="e")
            
            combobox = ttk.Combobox(self, values=options)
            combobox.current(i)  # Set initial selection
            # The combobox sticks to the left side of its cell.
            combobox.grid(row=i, column=1, padx=10, pady=5, sticky="w")
            self.comboboxes.append(combobox)
            
            # Create a new BooleanVar for each checkbox
            checkbox_var = tk.BooleanVar(value=self.app.legacy_gauge_selection[i])
            checkbox = tk.Checkbutton(self, variable=checkbox_var)
            checkbox.grid(row=i, column=2, sticky="w")
            # Store the BooleanVar, not the Checkbutton widget itself
            self.legacy_gauge_checkbox.append(checkbox_var)
    ...
    def save_and_back(self):
        ...
        new_checkbox_states = [cb_var.get() for cb_var in self.legacy_gauge_checkbox]
        self.app.legacy_gauge_selection = new_checkbox_states
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.legacy_gauge_selection = [False] * 4
