...
class CarDashboard(tk.Tk):
    def __init__(self, names, values, ranges, units):
        ...
        # List to store maximum values for each gauge
    ...
    def create_gauge(self, index, title):
        ...
        # Create a label to show the max value
        max_value_label = tk.Label(frame, text=f"Max: {value} {self.gauge_units[index]}")
        max_value_label.pack()

        # Store the canvas, value label, and max value label
        self.gauges.append((canvas, value_label, max_value_label))
    ...
    def update_gauge(self, values):
        for index in range(len(self.gauge_titles)):
            ...
            canvas, value_label, max_value_label = self.gauges[index]
            ...
            # Update max value if the current value is higher
...
if __name__ == "__main__":
    ...
    app = CarDashboard(fake_obd_pro.names, fake_obd_pro.getQuerryOutput(), ranges, units)
    fake_obd_pro.displayLoop(app)
    app.mainloop()
